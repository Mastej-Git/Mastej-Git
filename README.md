<h1 align="left">Hi there, I'm Michał</h1>



<ul>
  <li>Automatic Control and Robotics Student at Wroclaw University of Science and Technology <a href="https://wefim.pwr.edu.pl/en/">@PWR</a></li>
  <li>Part time developer at  <a href="https://nokiawroclaw.pl/">@Nokia</a></li>
  <li>Software engineer in Student Interest Group <a href="https://konar.pwr.edu.pl/">@KoNaR</a></li>
  <li>Enthusiast of Control Theory and Event Based Control with topics like: Multiple Mobile Robot System, Hybrid Control, Discrete Event System, Continuous Time System</li>
</ul>

<h3 align="left">🛠️ Tech Stack</h4>

<ul>
  <li><strong>Core:</strong> C/C++ · Python · Qt</li>
  <li><strong>Robotics</strong> MatLab · Simulink · ROS2</li>
  <li><strong>Embedded:</strong> Raspberry Pi · STM32
  <li><strong>Infrastructure:</strong> Docker · Linux · Git</li>
  <li><strong>Web development:</strong> FastAPI · TailwindCSS · HTMX · JavaScript</li>
</ul>

<h3 align="left">🤝🏻Connect with me:</h3>
<ul>
  <li><a href="https://www.linkedin.com/in/micha%C5%82-mastej-231248259/">@LinkedIn</a></li>
  <li><a href="mailto:m.mastej1812@gmail.com">m.mastej1812@gmail.com</a></li>
</ul>
